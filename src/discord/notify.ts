...
export const generateMachineNotificationMessage = (
  ...
  content: `${BELL_EMOJI} ${toRoomName(location.rooms.find(r => r.roomId === sub.room_id)!)}: **${capitalize(machine.type)} #${sub.machine_id}** is now **finished**!`,
});
...
export const generateNextAvailableNotificationMessage = (
  ...
  return {
    ...
    content: `${BELL_EMOJI} ${toRoomName(location.rooms.find(r => r.roomId === sub.room_id)!)}: **${available.length}** ${DBMachineTypeMap[sub.machine_type]}${available.length > 1 ? 's are' : ' is'} now available (${availableIDs}).`,
  };
};
